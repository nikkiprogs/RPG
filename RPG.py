import django
import time
import random
import hashlib
import os, sys

class Character:
    def __init__(self, name, attack, health):
        self.name = name
        self.attack = attack
        self.health = health
        self.max_health = health

    def __str__(self):
        return f'Имя🙎: {self.name} | Урон🗡 : {self.attack} | Здоровье✳️: {self.health}.'
    
    def attacked(self, other, ur):
        print(f'{self.name} атакует 🗡  {other.name}!')
        other.take_damage(self.attack, ur)

    def take_damage(self, damage, ur):
        self.health -= damage
        if self.health < 0:
            self.health = 0

    def vosst(self):
        self.health = self.max_health

class Warrior(Character):
    def __init__(self, name, attack, health, buff):
        self.name = name
        self.attack = attack
        self.health = health
        self.max_health = health
        self.buff = buff

    def __str__(self):
        return f'Имя🙎: {self.name} | Урон🗡 : {self.attack} | Здоровье✳️: {self.health} | Навык🔥: {self.buff}.'
    
    def attacked(self, other, ur):
        print(f'{self.name} атакует 🗡  {other.name}!')
        other.take_damage(self.attack, ur)
        chance = random.randrange(101)
        if 1 <= ur <= 249 and chance <= 25:
            other.take_damage(self.attack, ur)
            print('Воин нанес двойной урон!🗡🗡')
        if 250 <= ur <= 999 and chance <= 30:
            other.take_damage(self.attack, ur)
            print('Воин нанес двойной урон!🗡🗡')
        if 1000 <= ur <= 4999 and chance <= 35:
            other.take_damage(self.attack, ur)
            print('Воин нанес двойной урон!🗡🗡')
        if 5000 <= ur <= 9999 and chance <= 40:
            other.take_damage(self.attack, ur)
            print('Воин нанес двойной урон!🗡🗡')
        if ur >= 10000 and chance <= 45:
            other.take_damage(self.attack, ur)
            print('Воин нанес двойной урон!🗡🗡')
    
    
class Palladin(Warrior):
    def attacked(self, other, ur):
        print(f'{self.name} атакует 🗡  {other.name}!')
        other.take_damage(self.attack, ur)
        chance = random.randrange(101)
        if 1 <= ur <= 249 and chance <= 30:
            self.health += 20
            print('Палладин получил броню!🛡')
        if 250 <= ur <= 999 and chance <= 35:
            self.health += 25
            print('Палладин получил броню!🛡')
        if 1000 <= ur <= 4999 and chance <= 40:
            self.health += 30
            print('Палладин получил броню!🛡')
        if 5000 <= ur <= 9999 and chance <= 45:
            self.health += 35
            print('Палладин получил броню!🛡')
        if ur >= 10000 and chance <= 50:
            self.health += 40
            print('Палладин получил броню!🛡')

class Magic(Warrior):
    def attacked(self, other, ur):
        print(f'{self.name} атакует 🗡  {other.name}!')
        other.take_damage(self.attack, ur)
        chance = random.randrange(101)
        if 1 <= ur <= 249 and chance <= 20:
            self.health += 35
            print('Маг исцелился!💊')
        if 250 <= ur <= 999 and chance <= 25:
            self.health +=40
            print('Маг исцелился!💊')
        if 1000 <= ur <= 4999 and chance <= 30:
            self.health += 50
            print('Маг исцелился!💊')
        if 5000 <= ur <= 9999 and chance <= 35:
            self.health +=55
            print('Маг исцелился!💊')
        if ur >= 10000 and chance <= 40:
            self.health +=60
            print('Маг исцелился!💊')

class Witch(Warrior):
    def attacked(self, other, ur):
        print(f'{self.name} атакует 🗡  {other.name}!')
        other.take_damage(self.attack, ur)
        chance = random.randrange(101)
        if 1 <= ur <= 249 and chance <= 20:
            other.poison()
            print('Ведьма отравила соперника!🧪')
        if 250 <= ur <= 999 and chance <= 25:
            other.poison()
            print('Ведьма отравила соперника!🧪')
        if 1000 <= ur <= 4999 and chance <= 30:
            other.poison()
            print('Ведьма отравила соперника!🧪')
        if 5000 <= ur <= 9999 and chance <= 35:
            other.poison()
            print('Ведьма отравила соперника!🧪')
        if ur >= 10000 and chance <= 40:
            other.poison()
            print('Ведьма отравила соперника!🧪')

class Podrazhatel(Warrior):
    def take_damage(self, damage, ur):
        chance = random.randrange(101)
        if 1 <= ur <= 249 and chance <= 20:
            print('Подражатель увернулся!🪞')
        if 250 <= ur <= 999 and chance <= 25:
            print('Подражатель увернулся!🪞')
        if 1000 <= ur <= 4999 and chance <= 30:
            print('Подражатель увернулся!🪞')
        if 5000 <= ur <= 9999 and chance <= 35:
            print('Подражатель увернулся!🪞')
        if ur >= 10000 and chance <= 40:
            print('Подражатель увернулся!🪞')
        else:
            self.health -= damage
            if self.health < 0:
                self.health = 0

class Vampire(Warrior):
   def attacked(self, other, ur):
        print(f'{self.name} атакует 🗡  {other.name}!')
        other.take_damage(self.attack, ur)
        chance = random.randrange(101)
        if 1 <= ur <= 249 and chance <= 20:
            self.health += self.attack
            print('Вампиризм применен!🧛')
        if 250 <= ur <= 999 and chance <= 25:
            self.health += self.attack
            print('Вампиризм применен!🧛')
        if 1000 <= ur <= 4999 and chance <= 30:
            self.health += self.attack
            print('Вампиризм применен!🧛')
        if 5000 <= ur <= 9999 and chance <= 35:
            self.health += self.attack
            print('Вампиризм применен!🧛')
        if ur >= 10000 and chance <= 40:
            self.health += self.attack
            print('Вампиризм применен!🧛')
    
class Monstr(Character):
    def poison(self):
        self.health -= self.health/100*40

def upp():
    global hero_spis, mon_boss_spis, monst_spis, warrior, witch, vampire, mag, palladin, podrazhatel
    if 1 <= ur < 100:
        goblin = Monstr('Гоблин', 35, 70)
        ork = Monstr('Орк', 30, 85)
        troll = Monstr('Тролль', 20, 115)
        demon_boss = Monstr('Босс Демон', 35, 130)
        ork_boss = Monstr('Босс Орк', 25, 150)
        palladin = Palladin('Палладин', 35, 90, 'Броня 20 с шансом 30%🛡')
        mag = Magic('Маг', 30, 100, 'Лечение на 35 с шансом 20%💊')
        witch = Witch('Ведьма', 25, 100, 'Отравление на 40% здоровья с шансом 20%🧪')
        warrior = Warrior('Воин', 40, 80, 'Критический урон (Х2) с шансом 25%🗡🗡')
        podrazhatel = Podrazhatel('Подражатель', 25, 100, 'Уклонение от атаки с шансом 20%🪞')
        vampire = Vampire('Вампир', 20, 80, 'Шанс вампиризма 20%🧛')
        monst_spis = [goblin, ork, troll]
        mon_boss_spis = [ork_boss, demon_boss]
        hero_spis = [palladin, mag, witch, warrior, podrazhatel, vampire]

    if 100 <= ur < 250:
        goblin = Monstr('Гном', 40, 80)
        ork = Monstr('Полурослик', 30, 95)
        troll = Monstr('Дварф', 20, 130)
        demon_boss = Monstr('Босс Гном', 40, 145)
        ork_boss = Monstr('Босс Дварф', 25, 170)
        palladin = Palladin('Палладин', 35, 100, 'Броня 20 с шансом 30%🛡')
        mag = Magic('Маг', 35, 115, 'Лечение на 35 с шансом 20%💊')
        witch = Witch('Ведьма', 25, 115, 'Отравление на 40% здоровья с шансом 20%🧪')
        warrior = Warrior('Воин', 45, 90, 'Критический урон (Х2) с шансом 25%🗡🗡')
        podrazhatel = Podrazhatel('Подражатель', 25, 115, 'Уклонение от атаки с шансом 20%🪞')
        vampire = Vampire('Вампир', 20, 90, 'Шанс вампиризма 20%🧛')
        monst_spis = [goblin, ork, troll]
        mon_boss_spis = [ork_boss, demon_boss]
        hero_spis = [palladin, mag, witch, warrior, podrazhatel, vampire]

    if 250 <= ur < 500:
        goblin = Monstr('Орк-гоблин', 45, 90)
        ork = Monstr('Гоблиноид', 35, 105)
        troll = Monstr('Троллеподобная нечисть', 25, 145)
        demon_boss = Monstr('Босс Гоблиноид', 45, 160)
        ork_boss = Monstr('Босс Нечисть', 30, 190)
        palladin = Palladin('Палладин', 40, 110, 'Броня 25 с шансом 35%🛡')
        mag = Magic('Маг', 35, 130, 'Лечение на 40 с шансом 25%💊')
        witch = Witch('Ведьма', 30, 130, 'Отравление на 40% здоровья с шансом 25%🧪')
        warrior = Warrior('Воин', 45, 100, 'Критический урон (Х2) с шансом 30%🗡🗡')
        podrazhatel = Podrazhatel('Подражатель', 30, 130, 'Уклонение от атаки с шансом 25%🪞')
        vampire = Vampire('Вампир', 20, 100, 'Шанс вампиризма 25%🧛')
        monst_spis = [goblin, ork, troll]
        mon_boss_spis = [ork_boss, demon_boss]
        hero_spis = [palladin, mag, witch, warrior, podrazhatel, vampire]

    if 500 <= ur < 1000:
        goblin = Monstr('Великан', 50, 100)
        ork = Monstr('Огр', 35, 115)
        troll = Monstr('Зверочеловек', 25, 160)
        demon_boss = Monstr('Босс Великан-огр', 50, 180)
        ork_boss = Monstr('Босс Рептилоид', 30, 210)
        palladin = Palladin('Палладин', 40, 125, 'Броня 25 с шансом 35%🛡')
        mag = Magic('Маг', 35, 145, 'Лечение на 40 с шансом 25%💊')
        witch = Witch('Ведьма', 30, 145, 'Отравление на 40% здоровья с шансом 25%🧪')
        warrior = Warrior('Воин', 50, 110, 'Критический урон (Х2) с шансом 30%🗡🗡')
        podrazhatel = Podrazhatel('Подражатель', 35, 145, 'Уклонение от атаки с шансом 25%🪞')
        vampire = Vampire('Вампир', 20, 110, 'Шанс вампиризма 25%🧛')
        monst_spis = [goblin, ork, troll]
        mon_boss_spis = [ork_boss, demon_boss]
        hero_spis = [palladin, mag, witch, warrior, podrazhatel, vampire]

    if 1000 <= ur < 2500:
        goblin = Monstr('Дракон', 55, 110)
        ork = Monstr('Дракон-гоблин', 40, 130)
        troll = Monstr('Бес', 30, 175)
        demon_boss = Monstr('Босс Демоноид', 55, 200)
        ork_boss = Monstr('Босс Драконид', 35, 230)
        palladin = Palladin('Палладин', 45, 150, 'Броня 30 с шансом 40%🛡')
        mag = Magic('Маг', 35, 160, 'Лечение на 50 с шансом 30%💊')
        witch = Witch('Ведьма', 30, 160, 'Отравление на 40% здоровья с шансом 30%🧪')
        warrior = Warrior('Воин', 50, 120, 'Критический урон (Х2) с шансом 35%🗡🗡')
        podrazhatel = Podrazhatel('Подражатель', 35, 160, 'Уклонение от атаки с шансом 30%🪞')
        vampire = Vampire('Вампир', 25, 120, 'Шанс вампиризма 30%🧛')
        monst_spis = [goblin, ork, troll]
        mon_boss_spis = [ork_boss, demon_boss]
        hero_spis = [palladin, mag, witch, warrior, podrazhatel, vampire]

    if 2500 <= ur < 5000:
        goblin = Monstr('Джин', 60, 120)
        ork = Monstr('Нежить', 45, 140)
        troll = Monstr('Природный дух', 30, 190)
        demon_boss = Monstr('Босс Фея', 60, 220)
        ork_boss = Monstr('Босс Зомби', 40, 250)
        palladin = Palladin('Палладин', 45, 160, 'Броня 30 с шансом 40%🛡')
        mag = Magic('Маг', 40, 170, 'Лечение на 50 с шансом 30%💊')
        witch = Witch('Ведьма', 35, 170, 'Отравление на 40% здоровья с шансом 30%🧪')
        warrior = Warrior('Воин', 55, 130, 'Критический урон (Х2) с шансом 35%🗡🗡')
        podrazhatel = Podrazhatel('Подражатель', 35, 170, 'Уклонение от атаки с шансом 30%🪞')
        vampire = Vampire('Вампир', 25, 130, 'Шанс вампиризма 30%🧛')
        monst_spis = [goblin, ork, troll]
        mon_boss_spis = [ork_boss, demon_boss]
        hero_spis = [palladin, mag, witch, warrior, podrazhatel, vampire]

    if 5000 <= ur < 10000:
        goblin = Monstr('Лиса-нежить', 65, 130)
        ork = Monstr('Волк-гоблин', 50, 155)
        troll = Monstr('Ангел', 35, 205)
        demon_boss = Monstr('Босс Лорд', 65, 240)
        ork_boss = Monstr('Босс Королевский тролль', 45, 275)
        palladin = Palladin('Палладин', 50, 170, 'Броня 35 с шансом 45%🛡')
        mag = Magic('Маг', 40, 185, 'Лечение на 55 с шансом 35%💊')
        witch = Witch('Ведьма', 35, 185, 'Отравление на 40% здоровья с шансом 35%🧪')
        warrior = Warrior('Воин', 55, 145, 'Критический урон (Х2) с шансом 40%🗡🗡')
        podrazhatel = Podrazhatel('Подражатель', 40, 180, 'Уклонение от атаки с шансом 35%🪞')
        vampire = Vampire('Вампир', 30, 140, 'Шанс вампиризма 35%🧛')
        monst_spis = [goblin, ork, troll]
        mon_boss_spis = [ork_boss, demon_boss]
        hero_spis = [palladin, mag, witch, warrior, podrazhatel, vampire]
        
    if ur >= 10000:
        goblin = Monstr('Лесной эльф', 70, 140)
        ork = Monstr('Птицезмей', 60, 170)
        troll = Monstr('Темный эльф', 40, 230)
        demon_boss = Monstr('Босс Демоническая сущность', 70, 260)
        ork_boss = Monstr('Босс ???', 50, 300)
        palladin = Palladin('Палладин', 50, 180, 'Броня 40 с шансом 50%🛡')
        mag = Magic('Маг', 45, 200, 'Лечение на 60 с шансом 40%💊')
        witch = Witch('Ведьма', 40, 200, 'Отравление на 40% здоровья с шансом 40%🧪')
        warrior = Warrior('Воин', 60, 160, 'Критический урон (Х2) с шансом 45%🗡🗡')
        podrazhatel = Podrazhatel('Подражатель', 40, 200, 'Уклонение от атаки с шансом 40%🪞')
        vampire = Vampire('Вампир', 30, 160, 'Шанс вампиризма 40%🧛')
        monst_spis = [goblin, ork, troll]
        mon_boss_spis = [ork_boss, demon_boss]
        hero_spis = [palladin, mag, witch, warrior, podrazhatel, vampire]

def fight(heroy):
    time.sleep(1.5)
    global mon_boss_spis, monst_spis, ur
    if ur % 5 == 0:  monst = random.choice(mon_boss_spis)
    else: monst = random.choice(monst_spis)
    print('------------------------------\n' \
        f'⚔️Битва начинается! Твой соперник: {monst}⚔️')
    
    while True:
        print('------------------------------')
        heroy.attacked(monst, ur)
        time.sleep(1)
        print(f'Уровень здоровья монстра✳️: {monst.health}')
        time.sleep(1)
        if monst.health == 0:
            print(f'Монстр убит!🩸 Уровень {ur} пройден!✅')
            time.sleep(3.5)
            ur += 1
            monst.vosst()
            heroy.vosst()
            menu()
            break
        time.sleep(1)
        print('------------------------------')
        monst.attacked(heroy, ur)
        time.sleep(1)
        print(f'Уровень здоровья героя✳️: {heroy.health}')
        time.sleep(1)
        if heroy.health == 0:
            print(f'Герой убит.🩸 Уровень {ur} не пройден.❌')
            time.sleep(3.5)
            monst.vosst()
            heroy.vosst()
            menu()
            break
        time.sleep(1)

def menu():
    global ur, hero, bag
    upp()
    urr = ur - 1
    hero = hero_spis[bag]
    os.system('cls')
    print('------------------------------\n' \
    f'⚡️login | 🙎Персонаж: {hero}\n' \
    f'🔥Уровень: {urr} | ⚠️Место в рейтинге: "Заглушка"\n' \
    'Выберите, что вы хотите сделать:\n'\
    f'1. Сыграть {ur} уровень⚔️.\n' \
    '2. Выйти из игры❌')
    
    while True:
        vyb_menu = input('✅Выбор: ')
        if vyb_menu == '1':
            fight(hero)
            break
        elif vyb_menu == '2':
            print('До новых встреч, герой!🔥')
            time.sleep(2)
            exit()
            break
        else: 
            print('❌Ошибка! Введите цифру 1 или 2.❌')

if 'abc' == 'abc':
    print('Приветствую тебя, новичок!🌀 Ты попал в RPG-игру🔥, ' \
    'в которой можно сражаться с монстрами⚔️, которые меняются и ' \
    'становятся сильнее🦾 при прохождении уровней! Но не переживай, 1 из 6 героев🙎, ' \
    'которого ты выберешь, также будет становиться сильнее💪 и улучшать ' \
    'свою способность⚡️. Приступим к 1 этапу!✅️')
    time.sleep(12)
    global ur, hero, bag
    ur = 1
    upp()
    nn = 0
    for i in hero_spis:
        print(hero_spis[nn])
        nn += 1
        time.sleep(0.5)
    while True:
        pu = False
        try:
            time.sleep(1)
            bag = input('Выбери персонажа🙎 (обрати внимание, что после выбора персонажа🙎,' \
            ' его нельзя будет изменить!❌ Для выбора вводи только цифру): ')
            bag = int(bag) - 1
        except:
            print('❌Ошибка! Вводите только цифру выбранного персонажа🙎.❌')
            pu = True
        time.sleep(1)
        if bag == 0:
            hero = hero_spis[0]
            print(f'🙎Выбран герой: {hero}✅')
            break
        elif bag == 1:
            hero = hero_spis[1]
            print(f'🙎Выбран герой: {hero}✅')
            break
        elif bag == 2:
            hero = hero_spis[2]
            print(f'🙎Выбран герой: {hero}✅')
            break
        elif bag == 3:
            hero = hero_spis[3]
            print(f'🙎Выбран герой: {hero}✅')
            break
        elif bag == 4:
            hero = hero_spis[4]
            print(f'🙎Выбран герой: {hero}✅')
            break
        elif bag == 5:
            hero = hero_spis[5]
            print(f'🙎Выбран герой: {hero}✅')
            break
        else:
            if pu == False:
                print('❌Ошибка! Повторите ввод: ')
                time.sleep(1)
            else: None
    menu()

else:
    time.sleep(1)
    print('Охо-хо, это же ты! Давненько к нам не заглядывал, где пропадал?🤔 \n' \
    'Пойдем скорее, нужно сразиться с множеством монстров!⚔️')
    ur = 1 #получить с бд
    upp()
    time.sleep(1)
    menu()