...
class Witch(Warrior):
    def attacked(self, other, ur):
        ...
        if 1 <= ur <= 249 and chance <= 20:
            other.poison()
            print('Ведьма отравила соперника!🧪')
        if 250 <= ur <= 999 and chance <= 25:
            other.poison()
            print('Ведьма отравила соперника!🧪')
        if 1000 <= ur <= 4999 and chance <= 30:
...
def fight(heroy):
    time.sleep(1.5)
    global mon_boss_spis, monst_spis, urr, ur, row, vhod_name, vhod_pass, vhod_cod
    if urr % 5 == 0:  monst = random.choice(mon_boss_spis)
    else: monst = random.choice(monst_spis)
    print('------------------------------\n' \
        f'⚔️Битва начинается! Твой соперник: {monst}⚔️')
    
    while True:
        print('------------------------------')
        heroy.attacked(monst, urr)
        time.sleep(1)
        print(f'Уровень здоровья монстра✳️ : {monst.health}')
        time.sleep(1)
        if monst.health == 0:
            print(f'Монстр убит!🩸 Уровень {urr} пройден!✅')
            time.sleep(3.5)
            ur += 1
            cursor.execute('UPDATE Users SET level = ? WHERE name = ? AND pass = ? AND cod = ?', [ur, vhod_name, vhod_pass, vhod_cod])
            connect.commit()
            monst.vosst()
            heroy.vosst()
            menu()
        ...
        time.sleep(1)
        print('------------------------------')
        monst.attacked(heroy, urr)
        time.sleep(1)
        print(f'Уровень здоровья героя✳️ : {heroy.health}')
        time.sleep(1)
        if heroy.health == 0:
            print(f'Герой убит.🩸 Уровень {urr} не пройден.❌')
            time.sleep(3.5)
            monst.vosst()
            heroy.vosst()
            menu()
...
def menu():
    global urr, hero, bag, row, ur, vhod_name, vhod_pass, vhod_cod
    upp()
    hero = hero_spis[bag]
    urr = ur + 1
    cursor.execute('''
    ...
    cod = ?)''', [vhod_name, vhod_pass, vhod_cod])
    mesto = cursor.fetchone()[0]

    os.system('cls')
    print('------------------------------\n' \
    f'⚡️{row[1]} | 🙎Персонаж: {hero}\n' \
    f'🔥Уровень: {ur} | ⚠️  Место в рейтинге: {mesto}\n' \
    'Выберите, что вы хотите сделать:\n'\
    f'1. Сыграть {urr} уровень⚔️\n' \
    '2. Посмотреть рейтинг игроков⚠️\n' \
    '3. Выйти из игры❌\n' \
    '4. Удалить аккаунт❌\n')
    
    while True:
        vyb_menu = input('✅Выбор: ')
        if vyb_menu == '1':
            fight(hero)
            break
            ...
            os.system('cls')
            print('⚠️  Рейтинг лучших игроков⚠️')
            time.sleep(1)
            cursor.execute('SELECT name, pers, level FROM Users ORDER BY level DESC LIMIT 20')
            mesto = 1
            for i in cursor.fetchall():
                print(f'{mesto}. ' + str(i).replace('[', '').replace(']', '').replace('(', '').replace(')', '').replace("'", "") + ' уровень')
                mesto += 1
                time.sleep(0.4)
            propusk = input('(Нажмите Enter для выхода)')
            menu()

        elif vyb_menu == '3':
            print('До новых встреч, герой!🔥')
            time.sleep(2)
            exit()
            break

        elif vyb_menu == '4':
            dell = input('Ты уверен, что хочешь удалить аккаунт?❌  ')
            if dell == 'да' or 'Да':
                time.sleep(1)
                cursor.execute('DELETE FROM Users WHERE name = ? AND pass = ? AND cod = ?', [vhod_name, vhod_pass, vhod_cod])
                connect.commit()
                print('Выход из игры. Аккаунт удален.❌')
                time.sleep(2.5)
                exit()
                break
            else:
                print('Аккаунт сохранен.✅️')
                time.sleep(2)
                menu()
                break
...
vhod_name = input('Привет! ты зашел в RPG-игру. Если у тебя уже есть аккаунт, ' \
                  'вводи свои данные для входа. Если нет, введенные данные ' \
                    'будут использованы для регистрации.\n✅️Введи свое имя: ')
vhod_pass = input('✅️Введи пароль: ')
vhod_pass = vhod_pass.encode('utf-8')
vhod_pass = hashlib.sha256(vhod_pass)
vhod_pass = vhod_pass.hexdigest()
vhod_cod = input('✅️Введи кодовое слово: ')

polz = cursor.execute('SELECT * FROM Users WHERE name = ? AND pass = ? AND cod = ?', [vhod_name, vhod_pass, vhod_cod])
row = cursor.fetchone()

if row == None:
    print('Приветствую тебя, новичок!🌀 Ты попал в RPG-игру🔥, ' \
    ...
    'свою способность⚡️. Приступим к 1 этапу!✅️')
    time.sleep(12)
    global ur, hero, bag
    ur = 0
    upp()
    ...
    while True:
        ...
        try:
            time.sleep(1)
            bag = input('Выбери персонажа🙎 (обрати внимание, что после выбора персонажа🙎,' \
            ' его нельзя будет изменить!❌ Для выбора вводи только цифру): ')
            bag = int(bag) - 1
        except:
            print('❌Ошибка! Вводите только цифру выбранного персонажа🙎.❌')
            pu = True
        time.sleep(1)
        if bag == 0:
            hero = hero_spis[0]
            print(f'🙎Выбран герой: {hero}✅')
            cursor.execute('INSERT INTO Users(name, level, pers, pass, cod) VALUES(?, ?, ?, ?, ?)', [vhod_name, 0, hero.name, vhod_pass, vhod_cod])
            connect.commit()
            time.sleep(3)
            break
        elif bag == 1:
    ...
    polz = cursor.execute(f'SELECT * FROM Users WHERE name = ? AND pass = ? AND cod = ?', [vhod_name, vhod_pass, vhod_cod])
    row = cursor.fetchone()
    menu()

else:
    time.sleep(1)
    print('Охо-хо, это же ты! Давненько к нам не заглядывал, где пропадал?🤔 \n' \
    'Пойдем скорее, нужно сразиться с множеством монстров!⚔️')
    ur = int(row[2])
    hero_name = row[3]
